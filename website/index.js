...
new moduleLoader().init(config,function(err, lib){
	...
	const useSession = session({
		...
		secret: "-9chraminingtosisgreatandWill#3i2AlwaysRule@$%^m3iningw9823j",
		cookie: {
			maxAge: 3600000
		},
	...
	app.get('/', function(req, res){
		res.send(fs.readFileSync(__dirname + "/public/clientv2/dist/index.html").toString());
	});

	//serve static files for the Client
	app.use(express.static('public/clientv2/dist'));
